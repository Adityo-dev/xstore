  ...
  return (
    <section className="py-12 md:py-20 px-4 sm:px-6 lg:px-8 max-w-[1400px] mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-end mb-8 md:mb-12 gap-4">
        ...
        <Link
          href="/shop"
          className="text-primary hover:text-white transition-colors uppercase tracking-widest text-sm font-semibold border-b border-primary hover:border-white pb-1"
        >
          View All Categories
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {categories.map((category) => (
          <Link
            key={category.id}
            href={category.link}
            className="group relative flex flex-col items-center text-center p-8 rounded-xl bg-[#131417] border border-gray-800 hover:border-primary transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary/10 overflow-hidden"
          >
            ...
            <div className="relative z-10">
              {category.icon}
              <h3 className="text-white font-medium text-lg md:text-xl uppercase tracking-wide group-hover:text-primary transition-colors">
                {category.title}
              </h3>
